...
bipolar_dge = read.table("data/sampled_3k_cells.dat")
...
percent.mito <- colSums(bp@raw.data[mito.genes, ]) / colSums(bp@data)
...
```{r regress}
bp <- ScaleData(object = bp, vars.to.regress = c("nUMI", "percent.mito", "batchid"), genes.use = bp@var.genes, model.use='negbinom')
```
```{r}
