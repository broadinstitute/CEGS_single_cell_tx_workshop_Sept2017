...
valid_cells = valid_cells[genes_per_cell >= 500 & genes_per_cell <= 1400]  # set values based on your evaluation above
...
valid_cells = valid_cells[valid_cells %in% names(pct_mito)[pct_mito <= 10]]
...
```{r var_genes}
seurat2obj <- FindVariableGenes(object = seurat2obj, mean.function = ExpMean, dispersion.function = LogVMR, x.low.cutoff = 0.2, x.high.cutoff = 4, y.cutoff = 1.0)
```
...
# defined in multilevel clustring on the basis of the PCA reduction (uses Wilcoxon test)
